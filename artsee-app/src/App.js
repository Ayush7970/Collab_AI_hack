...
function App() {
    ...
    const handleFindCollaborators = async (description) => {
        setIsLoading(true);
        try {
            const response = await fetch('http://localhost:5001/match_users', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ query: description })
            });
            if(!response.ok) throw new Error('Failed to get matches from backend');
            const result = await response.json();
            
            if (result.success && result.recommendations && result.recommendations.length > 0) {
                setCollaborators(result.recommendations);
            } else {
                setCollaborators([ // Fallback to placeholders
                    {user_id: 'p1', name: 'Placeholder 1', bio: 'A creative musician.'},
                    {user_id: 'p2', name: 'Placeholder 2', bio: 'A visionary filmmaker.'},
                    {user_id: 'p3', name: 'Placeholder 3', bio: 'A talented fashion designer.'},
                ]);
                alert(result.message || "Could not find matches, showing placeholders.");
            }
            closeModal('collaborate');
            setView('collaborators');
        } catch(err) {
            console.error("Error finding collaborators:", err);
            alert(`Failed to find collaborators. Error: ${err.message}`);
        } finally {
            setIsLoading(false);
        }
    };
